...
/**
 * lomuto_partition - Order a subset according to
...
/**
 * lomuto_quick_sort - Implement the quicksort algorithm through recursion.
...
void lomuto_quick_sort(int *array, size_t size, int left, int right)
{
	...
	if (right - left > 0)
	{
		...
		lomuto_quick_sort(array, size, left, part - 1);
		lomuto_quick_sort(array, size, part + 1, right);
	}
}
